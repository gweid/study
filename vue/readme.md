...
虚拟 DOM 就是使用 js 对象去描述浏览器的 DOM 节点



> 为什么需要引入虚拟 DOM，虚拟 DOM 的好处：

第一：可以减少人为的失误造成的频繁 DOM 操作，导致浏览器重绘回流次数增加，影响页面性能。并不是说虚拟 DOM 就不需要操作 DOM 了，它也是需要操作 DOM 的，只是可以将多个 DOM 操作合并，减少页面重绘回流。而认为操作 DOM，这个需要写代码的人专门去做优化合并 DOM 操作

第二：将浏览器的 DOM 抽象成 js 对象形式的虚拟 DOM，那么就可以利用 render 去转换成不同平台的代码，进行跨端开发

第三：虚拟 DOM 在生成真实 DOM 之前，还可以利用它进行一些兼容处理、优化处理等

以上三点才是引入虚拟 DOM 的真正的目的，而不仅仅是减少页面的重绘回流
...
1. 前端控制：a、不用后端帮助，路由表维护在前端； b、逻辑相对比较简单，比较容易上手

    **实现方案：**

    - 根据登录用户的账号，返回前端用户的角色
    - 前端根据用户的角色，跟路由表的 meta.role 进行匹配
    - 讲匹配到的路由形成可访问路由

    **具体代码逻辑：**

    - 把静态路由和动态路由分别写在 router.js
    - 在 vuex 维护一个 state，通过配角色来控制菜单显不显示
    - 新建一个路由守卫函数，可以在 main.js，也可以抽离出来一个文件
    - 侧边栏的可以从 vuex 里面取数据来进行渲染

2. 后端控制：a、相对更安全一点 b、路由表维护在数据库
    ...
    **具体代码逻辑：**
    ...
    - router.js 里面只放一些静态的路由，login、404 之类
    - 整理一份数据结构，存到表里
    - 从后端获取路由数据，写一个数据转换的方法，将数据转成可访问的路由
...
对于Vue来说，使用模板让它有了很多优化的空间，配合响应式机制可以让Vue可以精确地进行节点更新 请问这句话怎么理解呢？vue 和 react 的 diff 算法不是类似的吗？

> 模板是静态的，限制多的好处在于利于分析，而JSX太灵活了，不好确定哪个部分是一定不会重新渲染的。
>
> 模板对比jsx是有缺点是不灵活，优势就是利于优化。比如，在模板有v-if的节点，只有存在or不存在两种状态并且只依赖于你v-if里面写的表达式，有v-for的就是循环，只有表达式里的变量变化才会变，节点及其子节点没有任何关键字，那他必然是静态的，不会有变化，我每一个状态变化，那些节点会变哪些节点不需要动，预先标记一下就心里有数了。 但是jsx其实就是js，你可以随意写，对写的代码预分析非常困难，所以一般是直接render后拿到拿到一整个组件的虚拟dom，每次任何状态变化，都会导致整个组件重新render拿到一个新的虚拟dom进行diff。