...
> 可以支持跨域，但是需要做处理，例如加跨域头

Service Workers运行在浏览器背后，独立于任何网页，可以用来作为各个客户端（标签页）之间的中介
...
- 页面加载速度更快、结构化清晰、页面显示简洁，如果 table 的内容特别多，那么需要加载完所有 table 内容才会进行渲染而且每改动一个地方，整个 table 都会重新渲染
