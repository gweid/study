...
**第一次握手：**第一次：客户端发送 `SYN` 包和初始序号 `seq = x` 给服务，服务端由`SYN=1`知道，客户端要求建立联机

**第二次握手：**服务端收到 `SYN` 包后，将标识位 `SYN` 和 `ACK` 置为1，确认序号 `ack = x + 1`（用于验证客户端下次请求序号，如果不是 x+1，说明不是）, 初始序号 `seq = y` （服务端自己的序号）发送给客户端

**第三次握手：**客户端收到后，将标识位 `ACK` 置为1, 确认序号 `ack = y + 1`（用于验证服务端下次请求序号）, 自己的序号 `seq = x + 1`, 发送给服务端

- `seq`序号，用来标识从`TCP`源端向目的端发送的字节流，发起方发送数据时对此进行标记
- `ack`确认序号，只有`ACK`标志位为1时，确认序号字段才有效
...
- 第一次挥手：客户端发送一个FIN报文，报文中会指定一个序列号。此时客户端处于第一阶段等待状态，此时不会发送数据，但是会继续接收数据

- 第二次挥手：服务端收到FIN之后，会发送ACK报文，且把客户端的序列号值+1作为ACK报文的序列号值，表明已经收到客户端的报文了，此时服务端处于关闭等待状态，不会再接收数据，但会继续发送数据；客户端进入下一阶段等待状态

- 第三次挥手：当数据发送完，服务端也想断开连接了，和客户端的第一次挥手一样，发送FIN报文，且指定一个序列号。此时服务端处于最后确认的状态

- 第四次挥手：需要过一阵子以确保服务端收到自己的ACK报文之后才会进入关闭状态，服务端收到ACK报文之后，就关闭连接了。
